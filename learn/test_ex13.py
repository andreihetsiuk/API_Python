...
@pytest.mark.parametrize("user_agent, exp_values", user_data)
def test_ex13(user_agent, exp_values):
    response = requests.get("https://playground.learnqa.ru/ajax/api/user_agent_check",
                            headers={"user-agent": user_agent}
                            )

    headers = json.loads(response.text)
    assert 'platform' and 'browser' and 'device' in headers, "platform or browser or device is absent in headers"
    fact_values = {'platform': headers["platform"], 'browser': headers["browser"], 'device': headers["device"]}
    assert fact_values == exp_values, "Factual values are not equal to expected values"
